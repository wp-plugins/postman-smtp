...
class ApiClientTest extends BaseTest
{
  public function testClient()
  {
    ...
    $req = new Postman_Google_Http_Request('http://foo.com');
  ...
  /**
   * @expectedException Postman_Google_Auth_Exception
   */
  ...
  public function testIniConfig()
  {
    ...
    $this->assertEquals(
        'gjfiwnGinpena3',
        $config->getClassConfig('Postman_Google_Auth_OAuth2', 'client_secret')
    );
    $this->assertInternalType(
        'array',
        $config->getClassConfig('Postman_Google_IO_Abstract')
    );
    $this->assertEquals(
        100,
        $config->getClassConfig('Postman_Google_IO_Abstract', 'request_timeout_seconds')
    );
  }
}
